...
public:
	...
	{
		// Ternary operator
		value = v == "Gab" ? "Master" : "v"; // value = (est-ce que v = "Gab" ?) Si oui, value = master, sinon, value = v

		std::cout << "Constructed Example with name " << value << " !" << std::endl;
	}
};
...
private:
	...
	mutable int m_DebugCount; // Pour pouvoir garder la constance de GetName(), on met cette variable en mutable.
	Example ex;

public:
	Entity()
		: m_Name("No Name"), m_DebugCount(0), ex(Example())
	{
